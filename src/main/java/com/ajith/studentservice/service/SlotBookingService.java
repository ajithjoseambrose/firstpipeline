...
public class SlotBookingService {
    ...
    public SlotResponse bookSlot(int custId, int bookId){
        ResponseEntity<Book> book = restTemplate.exchange("http://localhost:8080/books/" + bookId, HttpMethod.GET,null, Book.class);
            if (book.getStatusCode().is2xxSuccessful()) {
                ...
                if (recievedBook.getAvailable().equals("true")) {
                    ...
                    ResponseEntity<Book> responseEntity = restTemplate.exchange("http://localhost:8080/books/update/" + bookId, HttpMethod.PUT, requestEntity, Book.class);
    ...
    public SlotBooking bookReturned(int slot_id){
        ...
        HttpEntity<Book> requestEntity = new HttpEntity<>(bookObject);
        ResponseEntity<Book> responseEntity = restTemplate.exchange("http://localhost:8080/books/update/" + book_id, HttpMethod.PUT, requestEntity, Book.class);
        Book bookResponseObject = responseEntity.getBody();
